...
\section{Union des données dans une nouvelle table}
    ...
    Afin d'exploiter ces données, nous avons créée une nouvelle table \textit{deaths} centralisant toutes les morts des différentes régions.
...
(
    ...
    SELECT * FROM eastmids UNION
    SELECT * FROM wales UNION
    SELECT * FROM westmids
)
    \end{lstlisting}

    Cette table est alors modifiée selon l'utilisation désirée (étude des causes de niveau 2, étude de la néoplasie, ...) afin
